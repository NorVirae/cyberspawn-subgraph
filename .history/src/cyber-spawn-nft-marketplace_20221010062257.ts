...
export function handleOfferCreated(event: OfferCreated): void {
  ...
  log.debug("data debug",[newNotification.from, newNotification.message, newNotification.date, newNotification.id])

  const oldUser =  User.load(event.params.owner.toHexString())
   

  if(!oldUser){
    const newUser = new User(event.params.owner.toHexString())
...
export function handleOfferPurchased(event: OfferPurchased): void {
  ...
  newNotification.date = event.block.timestamp.toString()
  newNotification.save()
  const oldUser =  User.load(event.params.buyer.toHexString())
   

  if(!oldUser){
    const newUser = new User(event.params.buyer.toHexString())
    newUser.createdAt = event.block.timestamp.toString()
    newUser.save()
    return
  }
